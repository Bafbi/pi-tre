...
describeJj("sillajje sync", () => {
	it("syncs the current session onto <rev>, creating a merge commit", async () => {
		...
		const sessionCommit = changeId(path, sessionBookmark(sessionId));
		...
		expect(notifications).toContainEqual(
			expect.objectContaining({
				...
				msg: expect.stringContaining(
					`session ${sessionKeyId(sessionId)} synced onto main`,
				),
			}),
		);
	...
	it("syncs a different active session via -s", async () => {
		const cwd = makeRunnerCwd();
		tempDirs.push(cwd);
		await setupJjRepo(cwd);
		...
		jj(["bookmark", "set", sessionBookmark(otherId), "-r", "@"], otherPath);
		jj(["new"], otherPath);
		const otherCommit = changeId(otherPath, sessionBookmark(otherId));
		...
		expect(notifications).toContainEqual(
			expect.objectContaining({
				type: "info",
				msg: expect.stringContaining(
					`session ${sessionKeyId(otherId)} synced onto main`,
				),
			}),
		);
	}, 15_000);
	...
	it("reports an error when syncing an archived session", async () => {
		const cwd = makeRunnerCwd();
		tempDirs.push(cwd);
		await setupJjRepo(cwd);
	...
	it("aborts with a conflict warning when the sync creates file-level conflicts", async () => {
		const cwd = makeRunnerCwd();
		tempDirs.push(cwd);
		await setupJjRepo(cwd);
		...
		expect(
			notifications.some(
				(n) => n.type === "info" && n.msg.includes("synced onto"),
			),
		).toBe(false);
	...
	it("shows help for a target-less invocation", async () => {
		const cwd = makeRunnerCwd();
		tempDirs.push(cwd);
		await setupJjRepo(cwd);
