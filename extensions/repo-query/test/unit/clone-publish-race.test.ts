...
// Gated to Windows: the heuristics under test are what make the heal-retry a
// Windows concern. The mock above lets the same test run anywhere.
describe.runIf(process.platform === "win32")(
	"publish heal-retry re-probe",
	() => {
		it("reuses a clone published while the empty target was healed", async () => {
			const tempspace = mkdtempSync(
				join(tmpdir(), "repo-query-heal-race-"),
			);
