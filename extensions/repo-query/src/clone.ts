...
async function publishClone(
	...
	try {
		...
		// Keep the filesystem cause: the target may be absent or unwritable,
		// in which case "must be removed" alone misdiagnoses the failure.
		const cause = errorDetail(publishError);
		return {
			status: "failed",
			reason: "invalidTarget",
			error: `Clone target '${cloneTarget}' is not a valid git clone and must be removed before retrying.${cause ? ` publish: ${cause}` : ""}`,
		};
	}
}
